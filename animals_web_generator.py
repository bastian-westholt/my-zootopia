...
def get_data(animals_data):
    ...
    for animal in animals_data:
        ...
        animals_data_str += f'  <div class="card__title">Name: {animal["name"]}</div>\n'
        animals_data_str += f'  <p class="card__text">\n'
        animals_data_str += f'      <strong>Diet:</strong> {animal["characteristics"]["diet"]}<br/>\n'
        animals_data_str += f'      <strong>Location:</strong> {animal["locations"][0]}<br/>\n'
        if "type" in animal["characteristics"]:
            animals_data_str += f'      <strong>Type:</strong> {animal["characteristics"]["type"]}<br/>\n'
        animals_data_str += f'  </p>\n'
...
def replace_html_content(template_path, output_path, animals_data_str):
    ...
    for line in html_content:
        if '__REPLACE_ANIMALS_INFO__' in line:
            line = line.strip().replace('__REPLACE_ANIMALS_INFO__', animals_data_str)
